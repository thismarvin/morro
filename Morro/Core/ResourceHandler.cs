    ...
    class ResourceHandler<T> : IDisposable, IEnumerable<T>
    {
        ...
        public void Register(string name, T entry)
        {
            ...
            if (data.ContainsKey(formatedName))
                throw new MorroException("An entry with that name already exists; try a different name.", new ArgumentException("An item with the same key has already been added."));
        ...
        private void VerifyEntry(string name)
        {
            if (!data.ContainsKey(FormatName(name)))
                throw new MorroException("An Entry with that name has not been registered.", new KeyNotFoundException());
        }
