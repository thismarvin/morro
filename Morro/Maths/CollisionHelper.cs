...
namespace Morro.Maths
{
    ...
    {
        // This is really just for AABB's
        public static List<CollisionType> GetCollisionTypesBetween(IShape a, IShape b, Vector2 aVelocity)
        {
            ...
            MTV pass1 = CalculateMTV(a, b);
            MTV pass2 = CalculateMTV(b, a);

            if (pass1.Overlap == -1 || pass2.Overlap == -1)
                return result;

            MTV valid = pass1.Overlap < pass2.Overlap ? pass1 : pass2;

            Test(valid);

            return result;

            void Test(MTV target)
            {
                if (target.EdgeIndex % 2 == 0)
                {
        ...
        private static MTV CalculateMTV(IShape a, IShape b)
        {
