...
{
    static class ShapeSchemeHelper
    {
        public static ShapeScheme CreateShapeScheme(ShapeData shapeData, CPosition position, CDimension dimension, CTransform transform)
        {
            Matrix vertexTransform = CreateVertexTransform();
            Vector2[] vertices = CreateVertices();
            LineSegment[] lineSegments = CreateLineSegments();

            return new ShapeScheme(vertices, lineSegments);
